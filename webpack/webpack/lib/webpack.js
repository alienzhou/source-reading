...
const webpack = (options, callback) => {
	...
	// [tip] 单配置文件编译
		...
		// [tip] 初始化webpack默认配置，这里主要是进行配置的merge，返回最终的配置对象
		// [doubt] 仔细阅读OptionsDefaulter中的set方法与process方法（同时阅读WebpackOptionsDefaulter）
		options = new WebpackOptionsDefaulter().process(options);

		compiler = new Compiler(options.context);
		compiler.options = options;
		// [tip] 文件（input、output、watch、cache）相关配置
		new NodeEnvironmentPlugin().apply(compiler);
		// [tip] 执行所有插件的apply方法，将钩子进行注册
